...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    if (!user) {
      // User has a valid session cookie but is not in the DB yet.
      // This should not happen in normal flow (syncAuth0User creates the DB record).
      // Return forbidden so the frontend can re-run syncAuth0User.
