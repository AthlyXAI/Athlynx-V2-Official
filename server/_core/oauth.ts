/**
 * Auth0 OAuth Callback Handler — ATHLYNX
 * Handles /api/oauth/callback after Auth0 redirect.
 * Verifies the Auth0 JWT, upserts the user in Neon/MySQL, sets session cookie.
...
/**
 * Notify the owner of a new signup via SendGrid email only.
 * Replaces the old Manus notification service call.
 */
async function notifyOwnerOfSignup(opts: {
  name: string;
  email: string;
  loginMethod: string;
  signedUpAt: string;
  ...
  sendOwnerNewUserAlert(opts).catch((err: unknown) => {
...
export function registerOAuthRoutes(app: Application) {
  app.get("/api/oauth/callback", async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Send welcome email + owner alert to new users (fire-and-forget)
      if (isNewUser) {
        const name = userInfo.name || "Athlete";
        const signedUpAt = new Date().toLocaleString("en-US", { timeZone: "America/Chicago" });
        const trialStr = trialEndsAt?.toLocaleDateString("en-US") ?? "N/A";
