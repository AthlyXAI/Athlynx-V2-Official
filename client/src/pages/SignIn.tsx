...
import { signInWithGoogle, signInWithApple, signInWithFacebook, signInWithTwitter } from "@/lib/firebase";
...
export default function SignIn() {
  ...
  const handleSocial = async (provider: "google" | "apple" | "facebook" | "twitter") => {
    ...
    try {
      ...
      const sync = await syncFirebase.mutateAsync({
      ...
      navigate(sync.isNewUser ? "/onboarding" : "/feed");
    } catch (err: unknown) {
          ...
          <div className="w-full max-w-sm">
            ...
            {/* Login card */}
              ...
              {!showEmailForm ? (
                <>
                  {/* Google */}
                  <button onClick={() => handleSocial("google")} disabled={isLoading} className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-100 text-gray-800 font-bold py-3.5 px-6 rounded-xl active:scale-[0.98] transition-all duration-150 shadow-lg mb-3 disabled:opacity-60">
                  ...
                  {/* Apple */}
                  <button onClick={() => handleSocial("apple")} disabled={isLoading} className="w-full flex items-center justify-center gap-3 bg-black hover:bg-gray-900 border border-white/20 text-white font-bold py-3.5 px-6 rounded-xl active:scale-[0.98] transition-all duration-150 shadow-lg mb-3 disabled:opacity-60">
                  ...
                  {/* Facebook */}
                  <button onClick={() => handleSocial("facebook")} disabled={isLoading} className="w-full flex items-center justify-center gap-3 bg-[#1877F2] hover:bg-[#166FE5] text-white font-bold py-3.5 px-6 rounded-xl active:scale-[0.98] transition-all duration-150 shadow-lg mb-4 disabled:opacity-60">
                  ...
                  {/* X / Twitter */}
                  <button onClick={() => handleSocial("twitter")} disabled={isLoading} className="w-full flex items-center justify-center gap-3 bg-black hover:bg-gray-900 border border-white/20 text-white font-bold py-3.5 px-6 rounded-xl active:scale-[0.98] transition-all duration-150 shadow-lg mb-4 disabled:opacity-60">
                    {loading === "twitter" ? <div className="w-5 h-5 border-2 border-white/40 border-t-transparent rounded-full animate-spin" /> : <svg className="w-5 h-5" viewBox="0 0 24 24" fill="white"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>}
                    Continue with X
                  </button>
