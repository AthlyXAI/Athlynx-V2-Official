...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#1a3a8f] text-white">
      {/* ═══ INVESTOR SECTION — FULL IMMERSIVE EXPERIENCE ═══ */}
      <div style={{ background: "linear-gradient(180deg, #020818 0%, #060e24 40%, #0a1628 100%)", borderBottom: "2px solid rgba(59,130,246,0.4)" }}>

        {/* TOP TICKER */}
        <div style={{ background: "linear-gradient(90deg, #1e3a8a, #1e40af, #1e3a8a)", borderBottom: "1px solid rgba(99,179,237,0.3)" }} className="py-2 px-4 flex items-center justify-center gap-4 flex-wrap">
          <span className="flex items-center gap-2">
            ...
            <span className="text-emerald-400 text-xs font-black tracking-widest uppercase">LIVE PLATFORM</span>
          </span>
          <span className="text-blue-300 text-xs">|</span>
          <span className="text-white text-xs font-bold tracking-widest">PRE-SEED ROUND OPEN</span>
          <span className="text-blue-300 text-xs">|</span>
          <span className="text-cyan-300 text-xs font-bold">$15M POST-MONEY VALUATION</span>
          <span className="text-blue-300 text-xs">|</span>
          <span className="text-yellow-300 text-xs font-bold">$47B TOTAL ADDRESSABLE MARKET</span>
          <span className="text-blue-300 text-xs">|</span>
          <span className="text-white text-xs font-bold tracking-widest">SERIOUS INQUIRIES ONLY</span>
        </div>

        {/* HERO STATEMENT */}
        <div className="px-4 pt-10 pb-6 text-center max-w-4xl mx-auto">
          <div className="text-xs font-black tracking-[0.3em] text-cyan-400 uppercase mb-3">INVESTOR & STRATEGIC PARTNER OPPORTUNITY</div>
          <h2 className="text-3xl sm:text-5xl font-black text-white leading-tight mb-4" style={{ letterSpacing: "-0.02em" }}>
            The Platform That Owns
            <span style={{ background: "linear-gradient(90deg, #3b82f6, #06b6d4, #10b981)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}> Every Stage</span>
            <br />of an Athlete's Life.
          </h2>
          <p className="text-blue-200 text-base sm:text-lg font-medium max-w-2xl mx-auto leading-relaxed">
            AthlynXAI is not a concept. It is a fully deployed, revenue-ready platform with 75,662 lines of code, 142 features, dual AI engines, and 247 VIP users waiting for public launch. We built what no one else could.
          </p>
        </div>

        {/* STAT CARDS */}
        <div className="px-4 pb-8 grid grid-cols-2 sm:grid-cols-4 gap-3 max-w-4xl mx-auto">
          {[
            { num: "$47B", label: "Total Market", sub: "NIL + Sports Tech + Media", color: "#3b82f6" },
            { num: "$1.5M", label: "Pre-Seed Ask", sub: "10% equity · $15M valuation", color: "#10b981" },
            { num: "3X", label: "Projected Return", sub: "Within 36 months", color: "#f59e0b" },
            { num: "30X", label: "Exit Potential", sub: "$450M+ valuation path", color: "#ec4899" },
          ].map((s, i) => (
            <div key={i} className="rounded-2xl p-4 text-center" style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${s.color}40` }}>
        ...
        <div className="px-4 pb-8 max-w-4xl mx-auto">
          <div className="text-center mb-4">
            <span className="text-xs font-black tracking-widest text-blue-300 uppercase">— 5-Year Financial Projections —</span>
          </div>
              ...
              <thead>
                <tr style={{ background: "rgba(30,58,138,0.8)" }}>
                  {["Metric", "Year 1", "Year 2", "Year 3", "Year 4", "Year 5"].map(h => (
                    <th key={h} className="px-3 py-3 text-cyan-300 font-black text-left tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {[
                  { label: "Users", vals: ["10K", "35K", "100K", "400K", "1.5M"], highlight: false },
                  { label: "Revenue", vals: ["$450K", "$3.2M", "$15M", "$45M", "$135M"], highlight: true },
                  { label: "Gross Profit", vals: ["$400K", "$3M", "$14M", "$42M", "$126M"], highlight: false },
                  { label: "EBITDA", vals: ["-$1.2M", "-$800K", "$1.5M ✓", "$9M", "$41M"], highlight: true },
                  { label: "Net Income", vals: ["-$1.3M", "-$1M", "$750K", "$6M", "$29.25M"], highlight: false },
                ].map((row, ri) => (
                  <tr key={ri} style={{ background: ri % 2 === 0 ? "rgba(10,22,40,0.8)" : "rgba(13,31,60,0.8)" }}>
                    <td className="px-3 py-2.5 font-bold" style={{ color: row.highlight ? "#60a5fa" : "#94a3b8" }}>{row.label}</td>
                    {row.vals.map((v, vi) => (
                      <td key={vi} className="px-3 py-2.5 font-semibold" style={{ color: v.includes("-") ? "#f87171" : v.includes("✓") ? "#34d399" : "#e2e8f0" }}>{v}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
          ...
          <p className="text-center text-blue-400 text-[10px] mt-2">Source: AthlynXAI 5-Year P&L Model · Conservative Projections</p>
        </div>

        {/* FUNDING ROADMAP */}
        <div className="px-4 pb-8 max-w-4xl mx-auto">
          <div className="text-center mb-4">
            <span className="text-xs font-black tracking-widest text-blue-300 uppercase">— Funding Roadmap: Pre-Seed Through Series D —</span>
          </div>
          <div className="flex gap-2 overflow-x-auto pb-2">
            {[
              { stage: "PRE-SEED", ask: "$1.5M", eq: "10%", val: "$15M", color: "#3b82f6" },
              { stage: "SEED", ask: "$5M", eq: "12%", val: "$41.7M", color: "#06b6d4" },
              { stage: "SERIES A", ask: "$15M", eq: "10%", val: "$150M", color: "#10b981" },
              { stage: "SERIES B", ask: "$40M", eq: "8%", val: "$500M", color: "#f59e0b" },
              { stage: "SERIES C", ask: "$100M", eq: "7%", val: "$1.43B", color: "#ec4899" },
              { stage: "SERIES D", ask: "$250M", eq: "5%", val: "$5B", color: "#8b5cf6" },
            ].map((s, i) => (
              <div key={i} className="flex-shrink-0 rounded-xl p-3 text-center" style={{ minWidth: "110px", background: "rgba(255,255,255,0.04)", border: `1px solid ${s.color}50` }}>
                <div className="text-[9px] font-black tracking-widest mb-1" style={{ color: s.color }}>{s.stage}</div>
                <div className="text-white text-lg font-black">{s.ask}</div>
                <div className="text-blue-300 text-[10px]">{s.eq} equity</div>
                <div className="text-blue-400 text-[10px] mt-1">{s.val} val</div>
              </div>
            ))}
          </div>
        </div>

        {/* FOUNDER INVESTMENT HISTORY */}
        <div className="px-4 pb-8 max-w-4xl mx-auto">
          <div className="text-center mb-5">
            <span className="text-xs font-black tracking-widest text-blue-300 uppercase">— Founder Investment History: Dec 16, 2025 – Apr 30, 2026 —</span>
            <p className="text-blue-400 text-[11px] mt-1">Every dollar invested by Chad A. Dozier Sr. to build AthlynXAI from zero to live.</p>
          </div>
          <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(59,130,246,0.3)" }}>
            <table className="w-full text-xs">
              <thead>
                <tr style={{ background: "rgba(30,58,138,0.9)" }}>
                  {["Date", "Category", "Service / Item", "Amount"].map(h => (
                    <th key={h} className="px-3 py-3 text-cyan-300 font-black text-left tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {[
                  ["Dec 16, 2025", "AI Subscriptions", "Claude AI (Anthropic) — Initial Subscription", "$20.00"],
                  ["Dec 2025", "AI Subscriptions", "Manus AI — Platform Access & Credits", "$3,200+"],
                  ["Dec 2025", "Infrastructure", "Vercel — Hosting & Deployment", "$240/yr"],
                  ["Dec 2025", "Infrastructure", "GitHub — Organization & Repos", "$48/yr"],
                  ["Dec 2025", "Infrastructure", "Cloudflare — DNS, CDN & Security", "$240/yr"],
                  ["Dec 2025", "Database", "Neon PostgreSQL — Production Database", "$190/yr"],
                  ["Dec 2025", "Database", "PlanetScale — Failover Database", "$190/yr"],
                  ["Jan 2026", "Payments", "Stripe — Live Mode Setup & Fees", "$150+"],
                  ["Jan 2026", "AI Subscriptions", "Google Gemini 2.5 Flash — API Credits", "$500+"],
                  ["Jan 2026", "Domains", "athlynx.ai + 11 additional domains", "$1,200+"],
                  ["Jan 2026", "Email", "AWS SES — Transactional Email Service", "$120/yr"],
                  ["Jan 2026", "SMS", "AWS SNS — Toll-Free +18664502081", "$300+"],
                  ["Jan 2026", "Authentication", "Firebase — Google/Apple/Facebook/X Auth", "$240/yr"],
                  ["Feb 2026", "AI Subscriptions", "Manus AI — Continued Credits & Sessions", "$4,800+"],
                  ["Feb 2026", "Infrastructure", "Railway — Backend Hosting (migrated to Vercel)", "$180"],
                  ["Feb 2026", "Infrastructure", "Netlify — Static Hosting (migrated)", "$120"],
                  ["Feb 2026", "Payments", "Stripe — Subscription Infrastructure", "$200+"],
                  ["Mar 2026", "AI Subscriptions", "Claude AI — Continued Usage", "$1,800+"],
                  ["Mar 2026", "AI Subscriptions", "Manus AI — Continued Credits", "$4,200+"],
                  ["Mar 2026", "Infrastructure", "AWS — Cloud Infrastructure Setup", "$800+"],
                  ["Apr 2026", "AI Subscriptions", "Manus AI — Final Session Credits", "$3,600+"],
                  ["Apr 2026", "AI Subscriptions", "Nebius — Token Factory API (Llama-3.1)", "$200+"],
                  ["Apr 2026", "Legal", "Auth0/Okta — Enterprise Due Diligence", "$0 (pending)"],
                  ["Apr 2026", "Legal", "Anthropic Identity CIM — Due Diligence", "$0 (pending)"],
                  ["Apr 2026", "Infrastructure", "PlanetScale — PostgreSQL Migration", "$190"],
                  ["Apr 30, 2026", "AI Subscriptions", "Total AI Subscription Charges (documented)", "$20,000+"],
                ].map(([date, cat, item, amt], ri) => (
                  <tr key={ri} style={{ background: ri % 2 === 0 ? "rgba(10,22,40,0.8)" : "rgba(13,31,60,0.8)" }}>
                    <td className="px-3 py-2.5 text-blue-300 font-semibold whitespace-nowrap">{date}</td>
                    <td className="px-3 py-2.5" style={{ color: cat === "AI Subscriptions" ? "#a78bfa" : cat === "Infrastructure" ? "#60a5fa" : cat === "Payments" ? "#34d399" : cat === "Database" ? "#fb923c" : cat === "Domains" ? "#f472b6" : cat === "Legal" ? "#fbbf24" : "#94a3b8" }}>{cat}</td>
                    <td className="px-3 py-2.5 text-white">{item}</td>
                    <td className="px-3 py-2.5 font-black text-emerald-400 whitespace-nowrap">{amt}</td>
                  </tr>
                ))}
                <tr style={{ background: "rgba(30,58,138,0.9)" }}>
                  <td colSpan={3} className="px-3 py-3 font-black text-white text-sm">TOTAL FOUNDER INVESTMENT (Dec 2025 – Apr 2026)</td>
                  <td className="px-3 py-3 font-black text-yellow-300 text-sm">$50,000+</td>
                </tr>
                <tr style={{ background: "rgba(6,14,36,0.95)" }}>
                  <td colSpan={3} className="px-3 py-2 text-blue-400 text-[10px]">Traditional team equivalent (12 people, 17 months)</td>
                  <td className="px-3 py-2 font-black text-blue-300 text-[10px]">$2,074,000</td>
                </tr>
                <tr style={{ background: "rgba(6,14,36,0.95)" }}>
                  <td colSpan={3} className="px-3 py-2 text-blue-400 text-[10px]">Professional invoice value (Invoice DHG-2026-001)</td>
                  <td className="px-3 py-2 font-black text-blue-300 text-[10px]">$913,250</td>
                </tr>
                <tr style={{ background: "rgba(6,14,36,0.95)" }}>
                  <td colSpan={3} className="px-3 py-2 text-emerald-400 text-[10px] font-bold">TOTAL SAVINGS ACHIEVED</td>
                  <td className="px-3 py-2 font-black text-emerald-400 text-[10px]">$1,800,000+</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="px-4 pb-8 max-w-4xl mx-auto">
          <div className="text-center mb-5">
            <span className="text-xs font-black tracking-widest text-blue-300 uppercase">— Corporate Structure —</span>
            <p className="text-blue-400 text-[11px] mt-1">Founded November 2024 · Houston, TX · Chad A. Dozier Sr. & Glenn Tse met at Hope Lodge</p>
          </div>
          <div className="flex flex-col items-center gap-0">
            {/* Parent */}
            <div className="rounded-2xl px-6 py-4 text-center w-full max-w-sm" style={{ background: "linear-gradient(135deg, #1e3a8a, #1e40af)", border: "2px solid #3b82f6" }}>
              <div className="text-[10px] font-black tracking-widest text-cyan-300 mb-1">PARENT HOLDING COMPANY</div>
              <div className="text-white font-black text-lg">DOZIER HOLDINGS GROUP</div>
              <div className="text-blue-200 text-[11px]">Houston, TX · Est. November 2024</div>
              <div className="text-blue-300 text-[10px] mt-1">dozierholdingsgroup.com</div>
            </div>
            {/* Connector */}
            <div className="w-px h-6 bg-blue-500"></div>
            <div className="text-blue-400 text-[10px] font-bold tracking-widest">SUBSIDIARIES</div>
            <div className="w-px h-4 bg-blue-500"></div>
            {/* Subsidiaries grid */}
            <div className="grid grid-cols-2 gap-3 w-full">
              {[
                { name: "ATHLYNXAI", domain: "athlynx.ai", role: "AI Sports Platform", color: "#3b82f6", status: "LIVE" },
                { name: "SOFTMOR INC", domain: "softmor.ai", role: "AI Infrastructure", color: "#10b981", status: "ACTIVE" },
                { name: "NIL PORTAL", domain: "nilportal.ai", role: "NIL Ecosystem", color: "#f59e0b", status: "INTEGRATED" },
                { name: "DIAMOND GRIND", domain: "athlynx.ai/diamond-grind", role: "Baseball Platform", color: "#ec4899", status: "LIVE" },
                { name: "WARRIORS PLAYBOOK", domain: "athlynx.ai/warriors-playbook", role: "Football Platform", color: "#8b5cf6", status: "LIVE" },
                { name: "DHG EMPIRE", domain: "athlynx.ai/dhg", role: "Holdings Page", color: "#06b6d4", status: "LIVE" },
              ].map((sub, i) => (
                <div key={i} className="rounded-xl p-3" style={{ background: "rgba(255,255,255,0.04)", border: `1px solid ${sub.color}40` }}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[9px] font-black tracking-widest" style={{ color: sub.color }}>{sub.name}</span>
                    <span className="text-[8px] font-black px-1.5 py-0.5 rounded-full" style={{ background: `${sub.color}20`, color: sub.color }}>{sub.status}</span>
                  </div>
                  <div className="text-white text-[11px] font-bold">{sub.role}</div>
                  <div className="text-blue-400 text-[10px]">{sub.domain}</div>
                </div>
              ))}
            </div>
            {/* Team */}
            <div className="w-px h-6 bg-blue-500 mt-3"></div>
            <div className="text-blue-400 text-[10px] font-bold tracking-widest">LEADERSHIP TEAM</div>
            <div className="w-px h-4 bg-blue-500"></div>
            <div className="grid grid-cols-2 gap-2 w-full mt-1">
              {[
                { name: "Chad A. Dozier Sr.", role: "Founder & CEO", email: "cdozier14@athlynx.ai" },
                { name: "Glenn Tse", role: "CFO & COO", email: "gtse@athlynx.ai" },
                { name: "Andrew Kustes", role: "VP Technology", email: "akustes@athlynx.ai" },
                { name: "Lee Marshall", role: "VP Sales", email: "lmarshall@athlynx.ai" },
              ].map((m, i) => (
                <div key={i} className="rounded-xl p-3" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(59,130,246,0.2)" }}>
                  <div className="text-white text-[11px] font-black">{m.name}</div>
                  <div className="text-cyan-400 text-[10px]">{m.role}</div>
                  <div className="text-blue-500 text-[9px]">{m.email}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* SERIOUS INQUIRIES CTA */}
        <div className="px-4 pb-10 max-w-2xl mx-auto text-center">
          <div className="rounded-2xl p-6" style={{ background: "linear-gradient(135deg, rgba(30,58,138,0.6), rgba(6,14,36,0.9))", border: "2px solid rgba(59,130,246,0.6)" }}>
            <div className="text-xs font-black tracking-[0.3em] text-cyan-400 uppercase mb-1">SERIOUS INQUIRIES ONLY</div>
            <div className="text-[10px] text-blue-400 mb-4">Partners & Investors — We choose who we work with.</div>
            <h3 className="text-2xl font-black text-white mb-3">You Found the Right Platform.<br />At the Right Time.</h3>
            <p className="text-blue-200 text-sm mb-2 leading-relaxed">$50,000+ invested. 75,662 lines of code. 142 features. Dual AI engines. Live at athlynx.ai. We did not wait for funding to build. We built first. Now we are choosing who gets a seat at the table.</p>
            <p className="text-yellow-300 text-sm font-bold mb-5">&ldquo;Iron Sharpens Iron.&rdquo; &mdash; Proverbs 27:17</p>
            <div className="flex flex-col gap-3">
              <a href="mailto:cdozier14@athlynx.ai?subject=AthlynXAI%20Serious%20Investor%20Inquiry" className="block w-full py-4 rounded-xl font-black text-sm tracking-widest uppercase transition-all hover:scale-[1.02] active:scale-[0.98]" style={{ background: "linear-gradient(90deg, #1d4ed8, #0891b2)", color: "#fff", boxShadow: "0 0 30px rgba(59,130,246,0.4)" }}>
                REQUEST INVESTOR PACKAGE →
              </a>
              <Link href="/investor-hub">
                <div className="block w-full py-3 rounded-xl font-bold text-sm tracking-widest uppercase text-blue-300 hover:text-white transition-colors" style={{ border: "1px solid rgba(59,130,246,0.4)" }}>
                  VIEW FULL INVESTOR HUB
                </div>
              </Link>
            </div>
            <div className="mt-5 pt-4" style={{ borderTop: "1px solid rgba(59,130,246,0.2)" }}>
              <p className="text-white font-bold text-sm">Chad A. Dozier Sr. &mdash; Founder & CEO</p>
              <p className="text-blue-300 text-[11px] mt-1">cdozier14@athlynx.ai &nbsp;&middot;&nbsp; +1-601-498-5282 &nbsp;&middot;&nbsp; athlynx.ai</p>
              <p className="text-blue-500 text-[10px] mt-1">19039 Cloyanna Ln, Humble, TX 77346 &nbsp;&middot;&nbsp; Dozier Holdings Group</p>
              <p className="text-blue-600 text-[10px] mt-2">CONFIDENTIAL &mdash; For Serious Partners & Investors Only</p>
            </div>
          </div>
        </div>
